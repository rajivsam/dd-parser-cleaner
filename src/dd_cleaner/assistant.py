...
class CleaningAssistant:
    ...
    def augment_with_llm(self, profile: Dict[str, Any]) -> None:
        ...
        """
        # 🛡️ PRE-FLIGHT: Ensure Ollama is responsive before sending payload
        if not self._check_ollama_health():
            self.logger.warning("⚠️ LLM Augmentation skipped: Ollama is not responding to health checks.")
            return

        # ✂️ CONTEXT OPTIMIZATION: Truncate profile to only columns needing assessment.
        # Sending 100+ healthy columns to a CPU-bound LLM with 4k context causes timeouts.
        optimized_profile = {
            "row_count": profile.get("row_count"),
            "columns": {}
        }
        
        full_columns = profile.get("columns", {})
        for col, stats in full_columns.items():
            # Only include columns with nulls, mixed types, or low cardinality (potential constants)
            if stats.get("null_ratio", 0) > 0 or stats.get("is_mixed_type") or stats.get("cardinality", 0) <= 1:
                optimized_profile["columns"][col] = stats

        if not optimized_profile["columns"]:
            self.logger.info("skipping LLM augmentation: all columns appear healthy based on heuristics.")
            return

        payload_size = len(json.dumps(optimized_profile))
        self.logger.info(f"🤖 Requesting LLM insights (Optimized payload: {payload_size / 1024:.2f} KB, {len(optimized_profile['columns'])} cols)...")
        
        prompt = self._assemble_recommendation_prompt(optimized_profile)
        ...
        try:
            ...
            self.logger.info(f"✅ LLM Recommendation augmentation complete in {duration:.2f}s")
        except httpx.TimeoutException:
            self.logger.error(f"❌ LLM Recommendation timed out after {self.llm_timeout}s. Check Ollama resources.")
    ...
    def _check_ollama_health(self) -> bool:
        """Pings the local Ollama instance to ensure it is responsive."""
        try:
            # Use 127.0.0.1 to avoid potential IPv6 resolution overhead on some Linux distros
    ...
    def _call_llm(self, prompt: str) -> str:
        ...
        try:
            ...
            if response.status_code == 500:
                 raise RuntimeError(
                     "Ollama Internal Server Error (500). This usually happens when VRAM is full "
                     "or the model is struggling to load. Try running 'ollama stop <model_name>'."
                 )
            if response.status_code != 200:
                raise RuntimeError(f"Ollama API error: {response.status_code} - {response.text}")
            return response.json().get("response", "{}")
        except httpx.ConnectError:
            raise RuntimeError("Could not connect to Ollama. Is the service running on localhost:11434?")
        except httpx.TimeoutException:
            raise  # Re-raise to be caught by augment_with_llm for specific logging
